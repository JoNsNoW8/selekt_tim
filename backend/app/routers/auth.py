...
from app.core.security import verify_password, create_access_token, hash_password # Added hashing import
...
async def register_worker(data: RegisterRequest, db: Session = Depends(get_db)):
    if data.invite_code != MASTER_INVITE_CODE:
        raise HTTPException(
            status_code=403, 
            detail="Pogrešan pozivni kod. Obratite se administratoru."
        )
